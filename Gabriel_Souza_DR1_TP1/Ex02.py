...
# Função para simular a ordenação manual das cartas utilizando Insertion Sort
def insertion_sort(cartas):
    # Simular a 2º mão pegando uma carta por vez e inserindo na posição correta na primeira mão
    for i in range(1, len(cartas)):
        carta_atual = cartas[i]  # A segunda mão pega a carta atual
        j = i - 1
        
        # Mover as cartas da primeira mão para frente, se forem maiores que a carta atual
        while j >= 0 and cartas[j] > carta_atual:
            cartas[j + 1] = cartas[j]  # Mover a carta para frente
            j -= 1
        
        # Inserir a carta da segunda mão na posição correta
